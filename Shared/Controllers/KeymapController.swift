...
class KeymapController {
    ...
    func execute(_ command: Command, location: CGPoint?) {
        ...
        case .beamUp:
            ...
            Universe.universe.reader?.send(content: cpBeam)
        case .beamDown:
        ...
        case .bomb:
            ...
            Universe.universe.reader?.send(content: cpBomb)
        case .cloakUp:
            guard Universe.universe.gameState == .gameActive else { return }
            let cpCloak = MakePacket.cpCloak(state: true )
            Universe.universe.reader?.send(content: cpCloak)
        ...
        case .coup:
            ...
            Universe.universe.reader?.send(content: cpCoup)
            
        case .detEnemy:
            guard Universe.universe.gameState == .gameActive else { return }
            let cpDetTorps = MakePacket.cpDetTorps()
            Universe.universe.reader?.send(content: cpDetTorps)
        ...
        case .detOwn:
            ...
            for count in 0..<8 {
                ...
                Universe.universe.reader?.send(content: cpDetMyTorps)
            }
            SoundController.soundController.play(sound: .detonate, volume: 0.5)
        case .dockingPermission:
            let me = Universe.universe.me
            let cpDockperm = MakePacket.cpDockperm(state: !players[me].dockok)
            Universe.universe.reader?.send(content: cpDockperm)
        case .information:
        ...
        case .setCourse:
            ...
            if let cpDirection = MakePacket.cpDirection(netrekDirection: netrekDirection) {
                Universe.universe.reader?.send(content: cpDirection)
            }
        ...
        case .toggleShields:
            ...
            if shieldsUp {
                ...
                Universe.universe.reader?.send(content: cpShield)
            } else {
        ...
        case .tractorPressorOff:
            ...
            Universe.universe.reader?.send(content: cpTractor)
            let cpPressor = MakePacket.cpPressor(on: false, playerID: 0)
            Universe.universe.reader?.send(content: cpPressor)
        ...
        case .orbit:
            ...
            Universe.universe.reader?.send(content: cpOrbit)
        ...
        case .repair:
            ...
            Universe.universe.reader?.send(content: cpRepair)
        case .fireLaser:
            guard Universe.universe.gameState == .gameActive else { return }
            ...
            Universe.universe.reader?.send(content: cpLaser)
        ...
        case .fireTorpedo:
            ...
            Universe.universe.reader?.send(content: cpTorp)
        case .firePlasma:
            guard Universe.universe.gameState == .gameActive else { return }
            ...
            Universe.universe.reader?.send(content: cpPlasma)
        case .quitGame:
            debugPrint("Quitting game")
            let cpQuit = MakePacket.cpQuit()
            Universe.universe.reader?.send(content: cpQuit)
        case .practiceRobot:
            debugPrint("Requesting practice robot")
            let cpPractice = MakePacket.cpPractice()
            Universe.universe.reader?.send(content: cpPractice)
        case .lockStarbasePlanet:
            ...
            if closestPlayerDistance < closestPlanetDistance {
                ...
                Universe.universe.reader?.send(content: cpPlayerLock)
            } else {
                ...
                Universe.universe.reader?.send(content: cpPlanetLock)
            }
    ...
    func setSpeed(_ speed: Int) {
        if let cpSpeed = MakePacket.cpSpeed(speed: speed) {
            Universe.universe.reader?.send(content: cpSpeed)
        }
    }
